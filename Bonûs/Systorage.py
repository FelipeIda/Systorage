...
def buscar_produto_por_id():
    ...
    if not termo.isdigit():
        exibir_mensagem("O ID deve ser um número inteiro.", "aviso")
        return
    conexao = Conexao()
    try:
...
def salvar_produto():
    if not validar_campos():
        exibir_mensagem("Todos os campos são obrigatórios!\nQuantidade deve ser um número inteiro.\nPreço deve ser um número válido.", "aviso")
        return
    nome = entry_nome.get()
...
def alterar_produto():
    if not validar_campos():
        exibir_mensagem("Todos os campos são obrigatórios!", "aviso")
        return
    selected_item = treeview.selection()
    if not selected_item:
...
tk.Button(id_frame, text="🔍", command=buscar_produto_por_id).pack(side="left", padx=5)
...
tk.Button(nome_frame, text="🔍", command=buscar_produto).pack(side="left", padx=5)
...
# Preço
tk.Label(tela_estoque, text="Preço Unitário:", font=("Arial", 12), bg="lightblue").grid(row=6, column=0, padx=10, pady=(10, 0), sticky="w")
...
def alternar_musica():
    ...
    if mutado:
        ...
        botao_mutar.config(text="🔊 Som")
        mutado = False
    else:
        pygame.mixer.music.pause()
        botao_mutar.config(text="🔇 Mudo")
        mutado = True

# Botão de mutar no canto inferior direito
botao_mutar = tk.Button(tela_estoque, text="🔊 Som", width=10, height=2, font=("Arial", 10), command=alternar_musica)
botao_mutar.place(relx=1.0, rely=1.0, anchor="se", x=-10, y=-10)
